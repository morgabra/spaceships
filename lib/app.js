...
function App(settings) {
  ...
  this.app.configure(function() {
    ...
    this.app.use(express.session({ secret: settings.secret, key: 'derpgame', store: this.cookiestore}));
  ...
  this.app.post('/update', ensureAuthenticated, function(req, res) {
    ...
    if (!(user && repo && branch)) {
      ...
      this.redis.publish('api:' + user, url, function(err, result) {
        if (err) {
  ...
  this.io.on('connection', function (socket) {
    ...
    rclient.psubscribe('tick:' + socket.handshake.user.username + '*');
    rclient.on('pmessage', function(wc, chan, msg) {
